...
def main():
  ...
  img_getdata = np.array(img.getdata(), np.uint8)
  img_flatten = img_getdata.flatten()
  print(img_flatten, img_flatten.shape)
  print(engine.get_input_tensor_shape())
  #for result in engine.ClassifyWithImage(img, top_k=3):
  result = engine.RunInference(img_flatten)
  print("Embedding vector is: {}, shape={}, inference time={} ms".format(result[1], result[1].shape, result[0]))
